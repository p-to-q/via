...
6. Decide each route's origin and destination from the task before placing nodes. Shared p and q are neither a default nor a visual goal. If every route truly shares both, let the renderer infer them. If any route starts from a different available state or intentionally produces a different useful outcome, declare `graph.terminals`. The renderer fixes their roles as `START` and `DONE`; your node labels provide the changing captions underneath.
