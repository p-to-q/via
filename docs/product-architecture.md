...
Each path surfaces three compact decision signals: estimated tokens, anticipated time, and a concise route summary that captures what makes the path useful. Token and time ranges share one boundary—from choosing the route to its next useful engineering deliverable—so they remain comparable. Tokens describe model workload rather than exact billing; minutes describe active agent wall-clock rather than team calendar time.
